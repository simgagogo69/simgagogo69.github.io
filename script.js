...
if (boss.phase < 3 && boss.hp <= boss.maxhp * 0.1) {
  ...
  if (!boss.phase3SpawnedBoss) {
    const bossPool = ["GUNGEAR", "Masquerade", "Necron", "Blinkblade", "Crucible"];
    const pick = bossPool[Math.floor(Math.random() * bossPool.length)];
...
function fireUnixBinaryShot(boss, player, customAngle) {
  ...
  fireEnemyBullet(
    ...
    angle, 7, 16, type === 0 ? "#00ffff" : "#ff00ff", 1000,
    { unixBinary: type }
  );
}
...
function fireUnixBinaryExplosion(x, y) {
  for (let b = 0; b < 12; b++) {
    fireEnemyBullet(x, y, b * Math.PI / 6, 6, 14, "#00ff99", 800, {
...
if (bullet.boomerang) {
  let targetX, targetY;
    if (bullet.boomerangLife === undefined) {
        bullet.boomerangLife = 120;
    }

    bullet.boomerangLife--;

    if (bullet.boomerangLife <= 0) {
        bullets.splice(i, 1);
        console.log("spliced");
    }
  if (bullet.state === "out") {
  ...
  for (let j = enemies.length - 1; j >= 0; j--) {
    ...
    if (
      ...
      takeEnemyDamage(enemy, (player.damage * 0.7) * crit);
      if (enemy.type === 'boomling') {
        boomExplosion(enemy.x + enemy.w / 2, enemy.y + enemy.h / 2);
      }
    }
